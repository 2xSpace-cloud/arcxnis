...
// UNE SEULE tentative de connexion propre
...
// Schéma pour stocker l'intégralité de vos joueurs dans un seul document JSON
...
async function getDatabaseStats() {
  ...
  // Sécurité si aucun joueur n'existe encore
  if (players.length === 0) {
    return { 
      totalPlayers: 0, 
      averageLevel: 0, 
  ...
  players.forEach((player) => { 
    if (player.class) classCounts[player.class] = (classCounts[player.class] || 0) + 1; 
  });
  ...
  return {
    ...
    // Ajout d'une valeur par défaut (0) pour éviter le bug de Math.max() vide
    highestLevel: Math.max(...players.map((p) => p.level || 0), 0),
    richestPlayer: Math.max(...players.map((p) => p.gold || 0), 0),
  };
}
