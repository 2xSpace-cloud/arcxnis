...
app.post('/auth/request-code', async (req, res) => {
  ...
  const targetPlayer = await getPlayer(id);if (!targetPlayer) {return res.status(404).json({ error: 'Aucun personnage trouvé pour cet ID.' });}const code = generateCode();try {await sendDM(id, [🏰 **Medieval Kingdom — Connexion au tableau de bord**,Votre code de connexion est : ## \${code}`, ⏱️ Expire dans 5 minutes.`].join('\n'));} catch (err) {return res.status(500).json({ error: 'Impossible d'envoyer le DM.' });}storeCode(id, code);res.json({ success: true });});app.post('/auth/verify-code', async (req, res) => {const { discordId, code } = req.body;if (!discordId || !code) {return res.status(400).json({ error: 'Identifiant ou code manquant.' });}const id = discordId.trim();const result = verifyCode(id, code);if (!result.ok) {return res.status(400).json({ error: result.error });}const player = await getPlayer(id);req.session.user = {id: player.id,name: player.name,class: player.class};res.json({ success: true, user: req.session.user });});app.post('/auth/logout', (req, res) => {req.session.destroy();res.json({ success: true });});// ─── API endpoints protégés pour le Dashboard ───────────────────────────app.get('/api/user/profile', requireAuth, async (req, res) => {try {const player = await getPlayer(req.session.user.id);if (!player) return res.status(404).json({ error: 'Joueur introuvable.' });res.json(player);} catch (error) {res.status(500).json({ error: 'Erreur lors du chargement du profil.' });}});app.get('/api/stats', async (req, res) => {try {const stats = await getDatabaseStats();res.json(stats);} catch (error) {res.status(500).json({ error: 'Erreur lors du chargement des statistiques.' });}});app.get('/api/ranking', async (req, res) => {try {const players = await getAllPlayers();const sortedPlayers = players.sort((a, b) => b.level - a.level || b.experience - a.experience);res.json(sortedPlayers);} catch (error) {res.status(500).json({ error: 'Erreur lors du chargement du classement.' });}});app.get("/profil/:id", async (req, res) => {try {const player = await getPlayer(req.params.id);if (!player) return res.status(404).json({ error: "Personnage introuvable" });res.json(player);} catch (error) {res.status(500).json({ error: "Erreur lors du chargement" });}});// 7. Démarrage du serveur Expressapp.listen(PORT, () => {console.log(Le serveur Express tourne sur le port ${PORT});});
